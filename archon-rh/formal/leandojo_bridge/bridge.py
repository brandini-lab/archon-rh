...
class LeanBridge:
    ...
    def start(self) -> None:
        ...
        if LeanProofSession is not None:
            ...
            raise RuntimeError("LeanDojo not available and toy fallback not allowed.")
    ...
    def apply_tactic(self, tactic: str) -> LeanGoalState:
        ...
        if LeanProofSession is not None and isinstance(self._session, LeanProofSession):
            ...
            state = self._session.apply_tactic(tactic, step=self._step, session_id=self._session_id)  # type: ignore[attr-defined]
